...
**[Uptime Kuma](https://github.com/louislam/uptime-kuma)** is a single-purpose uptime monitor. One container, SQLite database, [100+ built-in notification providers](https://github.com/louislam/uptime-kuma#-notification). No incident management, no on-call scheduling, no user assignment. ~86,000 GitHub stars, 150M+ Docker pulls, MIT licensed.

**[OneUptime](https://github.com/OneUptime/oneuptime)** is a full operations platform covering monitoring, incident lifecycle, on-call duty, status pages, logs, APM, traces, and workflow automation. Requires PostgreSQL, ClickHouse, and Redis. MIT licensed, self-hosted.
...
| License                              | MIT                          | MIT                                  | AGPLv3 (OSS), proprietary (Enterprise/Cloud)       |
...
| JSM native integration               | Yes - native (v2.3.2, [PR #6830](https://github.com/louislam/uptime-kuma/pull/6830)) | No native | Cloud IRM: Yes. OSS alerting: No        |
...
| Built-in notification channels       | 100+                         | ~8-10 (Slack, Teams, Email, Discord, Telegram) | ~20 contact point types in OSS alerting + Apprise-style via webhooks |
...
| Domain expiry             | No                        | Yes                                          | No built-in                                           |
...
| Jira Service Management (Ops)      | Yes - native (v2.3.2, [PR #6830](https://github.com/louislam/uptime-kuma/pull/6830)) | No native | OSS: No. Cloud IRM: Yes - native ITSM integration |
| Jira Software (project issues)     | No native integration        | No native integration             | OSS: No. Cloud IRM: Yes - native ITSM integration |
| GitHub Issues                      | No native integration        | No native integration             | OSS: No. Cloud IRM: Yes - native ITSM integration |
...
| Apprise (78+ services)             | Yes - built-in               | No                                | No                                                |
| SMS (Twilio)                       | Yes - native                 | Yes - via on-call policy          | No built-in; needs custom webhook                 |
| Webhook (generic)                  | Yes                          | Yes                               | Yes - contact point                               |

**Grafana OSS alerting contact points** ([verified v13 docs](https://grafana.com/docs/grafana/latest/alerting/configure-notifications/manage-contact-points/integrations/)): AlertManager, DingDing, Discord, Email, Google Hangouts Chat, Kafka REST Proxy, Line, Microsoft Teams, OpsGenie, PagerDuty, Prometheus Alertmanager, Pushover, Sensu, Slack, Telegram, Threema, VictorOps, Webhook, Weixin (WeCom).

**JSM in Grafana Cloud IRM:** The [IRM integrations docs](https://grafana.com/docs/grafana-cloud/alerting-and-irm/irm/configure/integrations/) and [IRM product page](https://grafana.com/products/cloud/irm/) list Jira, ServiceNow, GitHub, and other ITSM tools as supported integrations. This is in Grafana Cloud IRM only, not in self-hosted Grafana OSS alerting.
...
| JSM Ops Alerts - create alert on monitor down              | Yes - native (v2.3.2) | No native                   | No native            | Yes - native ITSM         |
| JSM Ops Alerts - auto-close on recovery                   | Yes - native         | No native                    | No native            | Yes - native ITSM         |
| Jira Software - create project issue from incident         | No                   | No                           | No                   | Yes - native ITSM         |
| GitHub Issues - create issue from incident                 | No                   | No                           | No                   | Yes - native ITSM         |
| ServiceNow integration                                     | No                   | No                           | No                   | Yes - native ITSM         |
...
**Grafana Cloud IRM ITSM:** Native two-way integrations with Jira (both JSM and Jira Software), GitHub Issues, and ServiceNow. Incidents in IRM can automatically create Jira tickets or GitHub issues and sync status back. See [IRM integrations docs](https://grafana.com/docs/grafana-cloud/alerting-and-irm/irm/configure/integrations/). This is a Grafana Cloud IRM feature only.
...
| Database          | SQLite - embedded             | PostgreSQL 18 + ClickHouse 26.4 + Redis 8.2                   | SQLite or PostgreSQL (self-host)  |
...
**Grafana full LGTM stack** ([Loki](https://grafana.com/oss/loki/) + Grafana + [Tempo](https://grafana.com/oss/tempo/) + [Mimir](https://grafana.com/oss/mimir/), self-hosted): ~4-6 containers, ~2-4 GB RAM, significant storage. This is comparable in complexity to the OneUptime stack.

## Pricing

| Tier                          | Uptime Kuma       | OneUptime              | Grafana OSS            | Grafana Cloud IRM         |
| ----------------------------- | ----------------- | ---------------------- | ---------------------- | ------------------------- |
| Free self-hosted              | Yes - fully free  | Yes - MIT, self-hosted | Yes - AGPLv3, free     | No - cloud account needed |
...
| Incident management cost      | Free (no feature) | Free (self-hosted MIT) | Free (no feature in OSS) | [$20/user/month + platform fee](https://grafana.com/pricing/) |
...
| JSM integration - native, no bridge                   | Yes                 | No                                      | No (OSS)            | Yes                                  |
...
**If JSM native + GitHub Issues + Jira Software are all required without any cloud dependency:** None of the three tools cover all three natively in a self-hosted setup. Grafana Cloud IRM is the only option covering all three, but it requires a cloud account and paid subscription beyond 3 users.

**If JSM native only (no GitHub Issues, no Jira Software) and self-hosted is required:** Uptime Kuma covers JSM natively. Trade-off is losing incident management, on-call routing, and user assignment.

**If incident lifecycle and user assignment are the priority and JSM is not a hard requirement:** OneUptime covers all of that self-hosted at no cost. JSM and GitHub Issues have no native support, but Slack does.
